<?php
class CC_WC_Go_Positive_Delete_WC_Products extends CC_WC_Go_Positive_Handler {
    ...
    public function handle() {
        ...
        foreach ($products->products as $product) {
            ...
            $post_meta_date_touched = get_post_meta( $product_id, 'go_positive_date_touched', true);
            ...
            if (!$post_meta_date_touched) {
                ...
                $api_date_touched = new DateTime($date_touched);

                if ($post_date_touched != $api_date_touched) {
                    $product->delete();
                    $this->log($product_id . ' deleted');
                    $this->audit($product_id . ' deleted');

                    $this->log('post_date_touched: ', $post_date_touched);
                    $this->log('api_date_touched: ', $api_date_touched);
                }
            }
